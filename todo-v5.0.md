...
- [x] KB-104: PDF/HTML — block-level math (`$$...$$`) now detected. Parser `extract_code_block` recognizes `$$` fences on own line; single-line `$$content$$` also handled. Emits `math_display` token. HTML: `.math-display` CSS centered block. PDF: centered mono text. Terminal: centered. Also fixed: table cells now get inline parsing applied (`$math$`, `**bold**`, `*italic*`, etc. work inside table cells). 18 new tests. Commits `20bb9e0`, `d176e2e`.
