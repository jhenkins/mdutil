...
### Phase 1: Shared SVG→Image Layer (KB-026a)
- [ ] Create `mdutil/export/svg_to_image.py`
- [ ] Implement `render_mermaid_png()` (wraps merman-cli `--outputFormat png`)
- [ ] Implement `get_svg_dimensions()` for dimension extraction
- [ ] Unit tests for merman-cli PNG rendering (flowcharts, sequence, state, etc.)
- [ ] Unit tests for dimension extraction
- [ ] Error handling for malformed mermaid / merman-cli failures

### Phase 2: PDF Mermaid Rendering (KB-026b)
- [ ] Extend PdfExporter to consume `"mermaid"` tokens
- [ ] For mermaid tokens: call `svg_to_image.render_mermaid_png()` → embed via `pdf.image()`
- [ ] Pass theme, background, scale, and fit_width parameters
- [ ] Page-break logic: check remaining vertical space vs diagram height before rendering
- [ ] Aspect-ratio preservation when scaling to fit page width
- [ ] Error fallback: render mermaid code block when conversion fails
- [ ] Integration tests for PDF export with mermaid diagrams

### Phase 3: Verification & Documentation (KB-026c)
- [ ] Run full test suite (382+ existing + new)
- [ ] Verify PDF output in browser (diagrams render correctly)
- [ ] Verify air-gapped operation (zero network calls)
- [ ] Update README with PDF mermaid export documentation
- [ ] Update specification document
- [ ] Update CHANGELOG with v4.1 entries
...
- **v4.1.0** release with PDF mermaid diagram export
